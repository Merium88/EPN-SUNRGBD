...
import provider
import tf_util
import modelnet40_dataset_sunrgbd#orig
...
parser.add_argument('--model', default='pointnet2_cls_edge_sunrgbd', help='Model name [default: pointnet2_cls_edge_sunrgbd]')
parser.add_argument('--log_dir', default='log', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=1024, help='Point Number [default: 1024]')
parser.add_argument('--max_epoch', type=int, default=25, help='Epoch to run [default: 251]')
parser.add_argument('--model_path', default='log/model_orig_sunrgbd_edge_new_ms_1.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
DATA_PATH = os.path.join('/home/mariam/pointnet2/data/sunrgbd/')
FLAGS.normal = False
TRAIN_DATASET = modelnet40_dataset_sunrgbd.ModelNetDataset(root=DATA_PATH, npoints=NUM_POINT, split='train', normal_channel=FLAGS.normal, batch_size=BATCH_SIZE)
TEST_DATASET = modelnet40_dataset_sunrgbd.ModelNetDataset(root=DATA_PATH, npoints=NUM_POINT, split='test', normal_channel=FLAGS.normal, batch_size=BATCH_SIZE)
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            labels_pl_anchor,labels_pl_anchor_theta = MODEL.placeholder_inputs_anchor(BATCH_SIZE, NUM_POINT)
            ...
            # Get model and loss 
            pred_class, pred_pose, pred_anchor, end_points, pred_anchor_theta = MODEL.get_model_edge_cnn(pointclouds_pl, labels_pl_edge, is_training_pl, bn_decay=bn_decay)
            ...
            MODEL.get_loss_anchor_theta(pred_anchor_theta, labels_pl_anchor_theta)
            losses_class_anchor_theta = tf.get_collection('losses_anchor_theta')
            total_loss_anchor_theta = tf.add_n(losses_class_anchor_theta, name='total_loss_anchor_theta')
            tf.summary.scalar('total_loss_anchor_theta', total_loss_anchor_theta)
            for l in losses_class_anchor_theta + [total_loss_anchor_theta]:
                tf.summary.scalar(l.op.name, l)
            correct_anchor_theta = tf.equal(tf.argmax(pred_anchor_theta, 1), tf.to_int64(labels_pl_anchor_theta))
            accuracy_anchor_theta = tf.reduce_sum(tf.cast(correct_anchor_theta, tf.float32)) / float(BATCH_SIZE)
            tf.summary.scalar('accuracy_anchor_theta', accuracy_anchor_theta)
            ...
            total_loss = total_loss_class + total_loss_pose + total_loss_anchor + total_loss_anchor_theta
        ...
        for epoch in range(MAX_EPOCH):
            ...
            #if epoch % 1 == 0:
            #    save_path = saver.save(sess, os.path.join(LOG_DIR, "model_orig_sunrgbd_edge_new_ms_1.ckpt"))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    cur_batch_label_anchor_theta = np.zeros((BATCH_SIZE), dtype=np.int32)
    cur_batch_label_edge = np.zeros((BATCH_SIZE,32,32,3*3))

    total_correct = 0
    total_correct_anchor = 0
    total_correct_anchor_theta = 0
    ...
    while TRAIN_DATASET.has_next_batch():
        ...
        cur_batch_label_anchor[0:bsize] = batch_label_anchor[:,0]
        cur_batch_label_anchor_theta[0:bsize] = batch_label_anchor[:,1]
        ...
        feed_dict = {ops['pointclouds_pl']: cur_batch_data,
                     ...
                     ops['labels_pl_anchor_theta']: cur_batch_label_anchor_theta,
                     ops['labels_pl_edge']: cur_batch_label_edge,
                     ops['is_training_pl']: is_training,}
        

        summary_class, step_class, _, loss_val_class, pred_val_class,loss_val_pose,pred_val_pose,loss_val_anchor,pred_val_anchor,loss_val_anchor_theta,pred_val_anchor_theta = sess.run([ops['merged'], ops['step'],ops['train_op'], ops['loss_class'], ops['pred_class'], ops['loss_pose'],ops['pred_pose'], ops['loss_anchor'],ops['pred_anchor'], ops['loss_anchor_theta'],ops['pred_anchor_theta']], feed_dict=feed_dict)
        ...
        correct_anchor = np.sum(pred_val_anchor[0:bsize] == batch_label_anchor[0:bsize,0])
        total_correct_anchor += correct_anchor
        total_seen_anchor += bsize
        loss_sum_anchor += loss_val_anchor
        
        scores_anchor_theta = softmax(pred_val_anchor_theta)
        pred_val_anchor_theta = np.argmax(pred_val_anchor_theta, 1)
        correct_anchor_theta = np.sum(pred_val_anchor_theta[0:bsize] == batch_label_anchor[0:bsize,1])
        total_correct_anchor_theta += correct_anchor_theta
        loss_sum_anchor_theta += loss_val_anchor_theta

        if ((batch_idx+1)%100 == 0) :
          log_string(' ---- batch: %03d ----' % (batch_idx+1))
          log_string('mean loss classification: %f' % (loss_sum_class / 100))
          ...
          log_string('accuracy regression ADD: %f' %(float(total_pose)/100))
          log_string('loss for each parameter: %f %f %f \n %f %f %f \n %f' %(float(total_diff_pose[0]/100),float(total_diff_pose[1]/100),float(total_diff_pose[2]/100),float(total_diff_pose[3]/100),float(total_diff_pose[4]/100),float(total_diff_pose[5]/100),float(total_diff_pose[6]/100)))
          log_string('mean loss: %f' % (loss_sum_pose / 100))
          
          log_string('mean loss anchor classification: %f' % (loss_sum_anchor / 100))
          log_string('accuracy anchor classification: %f' % (total_correct_anchor / float(total_seen_anchor)))
          log_string('mean loss anchor classification theta: %f' % (loss_sum_anchor_theta / 100))
          log_string('accuracy anchor classification theta: %f' % (total_correct_anchor_theta / float(total_seen_anchor)))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    cur_batch_label_anchor = np.zeros((BATCH_SIZE), dtype=np.int32)
    cur_batch_label_anchor_theta = np.zeros((BATCH_SIZE), dtype=np.int32)
    cur_batch_label_edge = np.zeros((BATCH_SIZE,32,32,3*3), dtype=np.int32)
    ...
    while TEST_DATASET.has_next_batch():
        ...
        pred_val_anchor_theta = np.argmax(pred_val_anchor_theta, 1)
        correct_anchor_theta = np.sum(pred_val_anchor_theta[0:bsize] == batch_label_anchor[0:bsize,1])
        total_anchor_theta += correct_anchor_theta
        loss_sum_anchor_theta += loss_val_anchor_theta
        ...
        if ((EPOCH_CNT+1)%1 == 0):
           results_class = "/home/mariam/pointnet2/data/sunrgbd/prediction_class_sunrgbd_edge_bayesian_ms.txt"
           results_bbox = "/home/mariam/pointnet2/data/sunrgbd/prediction_bbox_sunrgbd_edge_bayesian_ms.txt"
           f_class = open(results_class,"a+")
           f_bbox = open(results_bbox,"a+") 
     
           for l in range (bsize):
             f_class.write("%i,%s,%s,%i,%i,%i,%i,%i,%i\n" %(EPOCH_CNT,batch_label_id[l,0],batch_label_id[l,1],pred_val_class[l], batch_label_class[l],pred_val_anchor[l], batch_label_anchor[l,0],pred_val_anchor_theta[l], batch_label_anchor[l,1]))
             f_bbox.write("%i,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n" %(EPOCH_CNT,pred_val_pose[l,0],pred_val_pose[l,1],pred_val_pose[l,2],pred_val_pose[l,3],pred_val_pose[l,4],pred_val_pose[l,5],pred_val_pose[l,6],batch_label_pose[l,0],batch_label_pose[l,1],batch_label_pose[l,2],batch_label_pose[l,3],batch_label_pose[l,4],batch_label_pose[l,5],batch_label_pose[l,6]))
           f_class.close()
           f_bbox.close()
    ...
    log_string('eval mean loss anchor_theta: %f' % (loss_sum_anchor_theta / float(batch_idx+1)))
    log_string('eval accuracy anchor_theta: %f'% (total_anchor_theta / float(total_seen)))
